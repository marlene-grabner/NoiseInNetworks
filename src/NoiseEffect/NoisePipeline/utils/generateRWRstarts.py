...
def generateRWRstarts(random_seed_list, G):
    ...
    print(all_start_points)
    print(second_neighbors_start_points)

    return all_start_points


def generateRWRstartsOLD(random_seed_list, original_network_nx):
    number_of_starts = len(random_seed_list)
    degree_sequence = sorted(
        original_network_nx.nodes(),
        key=lambda n: original_network_nx.degree(n),
        reverse=True,
    )
    # Single node start points
    start_point_indices = np.linspace(
    ...
    second_neighbors_start_points = []
    for node in single_start_points:
        neighbors = list(original_network_nx.neighbors(node[0]))
        first_neighbors_start_points.append([node + neighbors])
        second_neighbors = []
        for first_neighbor in neighbors:
            second_neighbors.extend(original_network_nx.neighbors(first_neighbor))
        second_neighbors_start_points.append([node + neighbors + second_neighbors])

    all_start_points = single_start_points + first_neighbors_start_points
