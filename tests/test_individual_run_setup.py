...
from NoiseEffect.individual_run_setup import IndividualAnalysis
...
def test_create_identifier(sample_network_request, sample_noise_information):
    """
    Tests if the unique identifier is created correctly based on the network request.
    """
    analysis = IndividualAnalysis(sample_network_request, sample_noise_information)
...
@patch("NoiseEffect.individual_run_setup.NoisyNetworkRecovery")
@patch("NoiseEffect.individual_run_setup.PerturbedEdges")
@patch("NoiseEffect.individual_run_setup.OriginalNetwork")
def test_run_orchestration(
    ...
    mock_original_network_class.assert_called_once_with(sample_network_request)
